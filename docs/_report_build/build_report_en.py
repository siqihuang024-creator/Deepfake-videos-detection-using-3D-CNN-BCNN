...
<p>Splits are identity-disjoint, with an additional <b>donor constraint</b>: a forged video enters a subset only if <em>both</em> its target identity and its donor identity fall in that subset. DFD forgeries are named <span class="m">&lt;target&gt;_&lt;source&gt;__&lt;scene&gt;__&lt;id&gt;</span>, so both identities are recoverable from the filename. Splitting by video alone would leave 73.2% of the DFD test forgeries with their own source real video in the training set. The constraint discards 1103 forged videos in DFD (36%) and 14538 in CelebDF++ (27.3%).</p>
...
<div class="tw">
<table>
  <caption><b>Table 2.</b> Training configuration for the two stages. Both experiments share these settings; they differ only in dataset, input mode, and early-stopping patience (Table 3).</caption>
...
<div class="tw">
<table>
  <caption><b>Table 3.</b> The two experiments recorded here. Network structure, input resolution, clip length, optimiser and learning-rate schedule are identical between them.</caption>
...
<div class="tw">
<table>
  <caption><b>Table 4.</b> DFD split statistics. Label convention: 1 = real, 0 = fake. DFD contains 363 original videos and 3068 forgeries, recorded by 28 actors.</caption>
...
<div class="tw">
<table>
  <caption><b>Table 5.</b> Experiment 1 test results, 54 real / 54 fake, video-level. The threshold is calibrated on the validation split at 5% FPR. Stage B early-stopped at epoch 19, with a best validation AUROC of 0.6627 and a run mean of 0.6259.</caption>
...
<div class="tw">
<table>
  <caption><b>Table 6.</b> CelebDF++ split statistics. The test subset has a real-to-fake ratio of roughly 1:32.</caption>
  <thead><tr><th>Subset</th><th class="n">Real</th><th class="n">Fake</th><th class="n">Identities</th><th>Use</th></tr></thead>
  <tbody>
...
<div class="tw">
<table>
  <caption><b>Table 7.</b> Experiment 3 test results, 170 real / 5433 fake, of which 3 videos were skipped for a missing detection or a decode failure, leaving 169 real / 5431 fake scored. The threshold is the one stored with the checkpoint. The base rate of the test subset is 0.9698.</caption>
  <thead><tr><th>Metric</th><th class="n">Value</th><th>Note</th></tr></thead>
  <tbody>
...
''' + fig(EXP3_ROC, 7,
          'Experiment 3 test set, drawn from 5600 per-video anomaly scores. <b>(a)</b> Score distributions; because the real-to-fake ratio is 1:32 the vertical axis is density rather than count. <b>(b)</b> ROC curve, AUC 0.5284, recomputed independently from the same score file and in agreement with Table 7.') + '''
