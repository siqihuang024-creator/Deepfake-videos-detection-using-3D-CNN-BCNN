...
<div class="tw">
<table>
  <caption><b>表 8.</b> 留出身份线性探针读数，每次测量 60 条视频。「随机初始化」是同一架构但未经训练的提取器，每行一次抽样；「差值」为两者之差。在供体安全协议下，本工作没有可用作正对照的提取器，见下方说明。</caption>
...
<div class="tw">
<table>
  <caption><b>表 11.</b> 同一批测试分数在两种重采样方案下的 95% percentile 区间。区间估计对方案敏感；鉴于身份簇数少且两类分布不对称（DFD 3 个、CelebDF++ 12 个携带伪造的身份），<b>我们不把名义上的区间覆盖情况解读为高于随机的证据</b>。</caption>
